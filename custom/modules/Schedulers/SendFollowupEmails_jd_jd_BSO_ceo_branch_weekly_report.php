        ...
        if($result->num_rows > 0){
            while ($row = $db->fetchByAssoc($result)) {
                ...
                if (!empty($assignedEmail)) {
                    ...
                    $GLOBALS['log']->fatal('jd_jd_BSO_ceo_branch_weekly_report send Email to assigne'.$assignedEmail);
                }
                ...
                if (!empty($reportToEmail)) {
                    ...
                    $GLOBALS['log']->fatal('jd_jd_BSO_ceo_branch_weekly_report send Email to reportToEmail'.$reportToEmail);
                }
                ...
                if (!empty($row['escalation_officer_email'])) {
                    ...
                    $GLOBALS['log']->fatal('jd_jd_BSO_ceo_branch_weekly_report send Email to reportToEmail'.$escalation_officer_email);
                }
                ...
                if($row['jd_status'] == 'UnResolved'){
                    ...
                    $currentTime = date_format($TimeZone,'Y-m-d H:i:s');
                        ...
                        $updateQuery = "UPDATE jd_jd_BSO_ceo_branch_weekly_report SET last_email_sent = '$nextEmailTime' WHERE id = '$recordId'";
                        $db->query($updateQuery);
                        $GLOBALS['log']->fatal('jd_jd_BSO_ceo_branch_weekly_report update record for next email time'.$updateQuery);
                }
            }
            return;
        } else{
